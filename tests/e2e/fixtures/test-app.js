...
class LeptosWSTestApp {
    ...
    setupEventListeners() {
        ...
        this.rpcBroadcastBtn.addEventListener('click', () => this.rpcBroadcastTest());
    ...
    handleRpcResponse(data) {
        if (data.id && data.result) {
            this.addMessage('received', `RPC Response (${data.id}): ${JSON.stringify(data.result)}`);
        } else if (data.id && data.error) {
            this.addMessage('received', `RPC Error (${data.id}): ${JSON.stringify(data.error)}`);
        }
    }
    ...
    simulateConnectionQualityChange() {
        if (this.isConnected) {
            ...
            this.connectionQuality = Math.max(10, this.connectionQuality - Math.random() * 5);
            this.updateMetrics();
            
            // Trigger reconnect if quality is too low
            if (this.connectionQuality < 20) {
                this.addMessage('system', 'Connection quality low, triggering reconnect...');
                this.reconnect();
            }
        }
    }
