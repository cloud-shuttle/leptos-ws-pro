use leptos_ws_pro::{
    ...
    transport::{TransportError, ConnectionState},
    codec::JsonCodec,
    rpc::{RpcClient, RpcMethod, RpcError},
};
...
async fn test_real_websocket_connection() {
    ...
    assert!(result.is_err(), "Expected connection to fail since no server is running: {:?}", result);
    
    // Verify the error is a real WebSocket connection error, not a simulated one
    match result {
        Err(TransportError::ConnectionFailed(msg)) => {
            assert!(msg.contains("WebSocket connection failed"), "Expected real WebSocket error, got: {}", msg);
        }
    ...
    assert!(result.is_err(), "Expected send to fail when not connected: {:?}", result);
    ...
    assert!(received_msg.is_err(), "Expected receive to fail when not connected: {:?}", received_msg);
...
async fn test_websocket_connection_timeout() {
    ...
    assert!(result.unwrap().is_err(), "Connection to unreachable server should fail");
}
...
async fn test_websocket_message_ordering() {
    ...
    let messages = vec![
        TestMessage { id: 1, content: "First".to_string(), timestamp: 1 },
        TestMessage { id: 2, content: "Second".to_string(), timestamp: 2 },
        TestMessage { id: 3, content: "Third".to_string(), timestamp: 3 },
    ];
