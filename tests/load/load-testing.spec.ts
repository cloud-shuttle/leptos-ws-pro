...
test.describe("Load Testing and Performance", () => {
  test("should handle high message throughput", async ({ page }) => {
    ...
    console.log(
      `Sent ${messageCount} messages in ${duration}ms (${((messageCount / duration) * 1000).toFixed(2)} msg/s)`,
    );
  });
  ...
  test("should handle concurrent message sending", async ({ page }) => {
    ...
    for (let i = 0; i < messageCount; i++) {
      promises.push(
        page
          .fill("#message-input", `Concurrent message ${i}`)
          .then(() => page.click("#send-btn")),
      );
    }
  ...
  test("should handle RPC load testing", async ({ page }) => {
    ...
    // Log performance metrics
    console.log(
      `Sent ${rpcCount} RPC requests in ${duration}ms (${((rpcCount / duration) * 1000).toFixed(2)} req/s)`,
    );
  });
  ...
  test("should handle heartbeat stress testing", async ({ page }) => {
    ...
    // Log performance metrics
    console.log(
      `Sent ${heartbeatCount} heartbeats in ${duration}ms (${((heartbeatCount / duration) * 1000).toFixed(2)} hb/s)`,
    );
  });
  ...
  test("should handle connection quality degradation", async ({ page }) => {
    ...
    const initialQuality = await page
      .locator("#connection-quality")
      .textContent();
  ...
  test("should handle memory usage under load", async ({ page }) => {
    ...
    for (let i = 0; i < messageCount; i++) {
      await page.fill(
        "#message-input",
        `Memory test message ${i} - ${"A".repeat(100)}`,
      );
      await page.click("#send-btn");
...
test.describe("Performance Monitoring", () => {
  ...
  test("should monitor connection stability over time", async ({ page }) => {
    ...
    console.log(
      `Connection remained stable for ${monitoringDuration}ms, sent ${messageCount} messages`,
    );
  });
});
