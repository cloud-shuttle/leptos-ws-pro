...
async fn test_websocket_throughput() {
    ...
    println!(
        "Throughput: {:.2} MB/s, Total time: {:?}",
        throughput_mbps, total_time
    );
...
async fn test_transport_creation_performance() {
    ...
    for _ in 0..num_instances {
        let client = WebTransportConnection::new(wt_config.clone())
            .await
            .unwrap();
    ...
    assert!(
        ws_creation_time < Duration::from_millis(100),
        "WebSocket creation too slow: {:?}",
        ws_creation_time
    );
    assert!(
        sse_creation_time < Duration::from_millis(100),
        "SSE creation too slow: {:?}",
        sse_creation_time
    );
    assert!(
        wt_creation_time < Duration::from_millis(100),
        "WebTransport creation too slow: {:?}",
        wt_creation_time
    );

    println!(
        "WebSocket creation: {:?} for {} instances",
        ws_creation_time, num_instances
    );
    println!(
        "SSE creation: {:?} for {} instances",
        sse_creation_time, num_instances
    );
    println!(
        "WebTransport creation: {:?} for {} instances",
        wt_creation_time, num_instances
    );
}
...
async fn test_memory_usage_benchmark() {
    ...
    assert!(
        ...
        "Memory benchmark took too long: {:?}",
        total_time
    );
    ...
    println!(
        "Memory benchmark: {:.0} messages/sec, Total time: {:?}",
...
async fn test_adaptive_transport_performance() {
    ...
    assert!(
        ...
        "Adaptive transport creation too slow: {:?}",
        creation_time
    );
    ...
    println!(
        "Adaptive transport creation: {:?} for {} instances",
