...
async fn test_real_websocket_connection() {
    ...
    assert!(
        result.is_ok(),
        "Failed to connect to real WebSocket server: {:?}",
        result
    );
...
async fn test_real_websocket_binary_message() {
    ...
    assert!(
        received.is_ok(),
        "Timeout waiting for echoed binary message"
    );
...
async fn test_real_websocket_connection_failure() {
    ...
    match result.unwrap_err() {
        TransportError::ConnectionFailed(msg) => {
            assert!(
                msg.contains("connection") || msg.contains("refused") || msg.contains("timeout")
            );
        }
