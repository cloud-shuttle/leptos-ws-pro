...
async fn test_websocket_connection_establishment() {
    ...
    let ws_context = WebSocketContext::new_with_url(&("ws://127.0.0.1:".to_string() + &addr.port().to_string()));
...
async fn test_websocket_message_receiving() {
    ...
    let server_task = tokio::spawn(async move {
        ...
        write.send(tokio_tungstenite::tungstenite::Message::Text(msg_text)).await.unwrap();
